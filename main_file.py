...
def matrix_entry():
    ...
    lbl_title = Label(panel1, text = "ENTER THE PATH DISTANCE BETWEEN THE CITIES", font=(15),bg='blue')
    lbl_title.place(x=480,y=50)
    for i in range(5):
        l[i]=Label(panel1,text=city[i], font=(15),bg='blue')
    ...
    for i in range(5):
        ...
        for j in range(5):
            if i==j:
                e[k]=Entry(panel1,bd=5,state='disabled',bg='blue')
                e[k].place(x=350+i*150,y=150+j*30)
                k+=1
            else:
                e[k]=Entry(panel1,bd=5,bg='blue')
                e[k].place(x=350+i*150,y=150+j*30)
                k+=1
    b=Button(panel1,text="CALCULATE",command=matrix_copy_to_local, font=(15),bg='blue')
    b.place(x=610,y=350)
